...
func ldbWithGlobal(db *leveldb.DB, folder []byte, truncate bool, fn fileIterator) {
	...
	for dbi.Next() {
		...
		gk := globalKeyName(dbi.Key())
		fk := deviceKey(folder, vl.versions[0].device, gk)
		bs, err := snap.Get(fk, nil)
		if err != nil {
			l.Debugf("key: %q (%x)", dbi.Key())
			l.Debugf("vl: %v", vl)
			l.Debugf("gk: %q (%x)", gk, gk)
...
func CheckDatabase(db *leveldb.DB) (error, int) {
	runtime.GC()

	start := globalKey(nil, nil)
	limit := globalKey([]byte{0xff, 0xff, 0xff, 0xff}, []byte{0xff, 0xff, 0xff, 0xff})
	snap, err := db.GetSnapshot()
	if err != nil {
		panic(err)
	}
	...
	for dbi.Next() {
		name := globalKeyName(dbi.Key())
		folder := globalKeyFolder(dbi.Key())
		...
		if len(vl.versions) == 0 {
			return fmt.Errorf("Empty global version list for file %q / %q", folder, name), 0
		}

		prevVer := uint64(1<<64 - 1)
		for _, v := range vl.versions {
			if v.version > prevVer {
				l.Debugln(vl)
				return fmt.Errorf("Version list not in descending order for file %q / %q", folder, name), 0
			}
			prevVer = v.version

			fk := deviceKey(folder, v.device, name)
			_, err := snap.Get(fk, nil)
			if err != nil {
				var id protocol.DeviceID
				copy(id[:], v.device)
				return fmt.Errorf("File missing for global %q / %q @ %s", folder, name, id), 0
			}
