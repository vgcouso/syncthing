...
func NewFileSet(folder string, db *leveldb.DB) *FileSet {
	...
	s.db.ldbWithAllFolderTruncated([]byte(folder), func(device []byte, f FileInfoTruncated) bool {
...
func (s *FileSet) Replace(device protocol.DeviceID, fs []protocol.FileInfo) {
	...
	s.localVersion[device] = s.db.ldbReplace([]byte(s.folder), device[:], fs)
	if len(fs) == 0 {
...
func (s *FileSet) ReplaceWithDelete(device protocol.DeviceID, fs []protocol.FileInfo, myID uint64) {
	...
	s.mutex.Lock()
	defer s.mutex.Unlock()
	if lv := s.db.ldbReplaceWithDelete([]byte(s.folder), device[:], fs, myID); lv > s.localVersion[device] {
...
func (s *FileSet) Update(device protocol.DeviceID, fs []protocol.FileInfo) {
	...
	if device == protocol.LocalDeviceID {
		...
		for _, newFile := range fs {
			existingFile, ok := s.db.ldbGet([]byte(s.folder), device[:], []byte(newFile.Name))
	...
	if lv := s.db.ldbUpdate([]byte(s.folder), device[:], fs); lv > s.localVersion[device] {
		s.localVersion[device] = lv
	}
}
...
func (s *FileSet) WithNeed(device protocol.DeviceID, fn Iterator) {
	...
	s.db.ldbWithNeed([]byte(s.folder), device[:], false, nativeFileIterator(fn))
}
...
func (s *FileSet) WithNeedTruncated(device protocol.DeviceID, fn Iterator) {
	...
	s.db.ldbWithNeed([]byte(s.folder), device[:], true, nativeFileIterator(fn))
}
...
func (s *FileSet) WithHave(device protocol.DeviceID, fn Iterator) {
	...
	s.db.ldbWithHave([]byte(s.folder), device[:], false, nativeFileIterator(fn))
}
...
func (s *FileSet) WithHaveTruncated(device protocol.DeviceID, fn Iterator) {
	...
	s.db.ldbWithHave([]byte(s.folder), device[:], true, nativeFileIterator(fn))
}
...
func (s *FileSet) WithGlobal(fn Iterator) {
	...
	s.db.ldbWithGlobal([]byte(s.folder), nil, false, nativeFileIterator(fn))
}
...
func (s *FileSet) WithGlobalTruncated(fn Iterator) {
	...
	s.db.ldbWithGlobal([]byte(s.folder), nil, true, nativeFileIterator(fn))
}
...
func (s *FileSet) WithPrefixedGlobalTruncated(prefix string, fn Iterator) {
	...
	s.db.ldbWithGlobal([]byte(s.folder), []byte(osutil.NormalizedFilename(prefix)), true, nativeFileIterator(fn))
}

func (s *FileSet) Get(device protocol.DeviceID, file string) (protocol.FileInfo, bool) {
	f, ok := s.db.ldbGet([]byte(s.folder), device[:], []byte(osutil.NormalizedFilename(file)))
	f.Name = osutil.NativeFilename(f.Name)
	return f, ok
}

func (s *FileSet) GetGlobal(file string) (protocol.FileInfo, bool) {
	fi, ok := s.db.ldbGetGlobal([]byte(s.folder), []byte(osutil.NormalizedFilename(file)), false)
	if !ok {
...
func (s *FileSet) GetGlobalTruncated(file string) (FileInfoTruncated, bool) {
	fi, ok := s.db.ldbGetGlobal([]byte(s.folder), []byte(osutil.NormalizedFilename(file)), true)
	if !ok {
...
func (s *FileSet) Availability(file string) []protocol.DeviceID {
	return s.db.ldbAvailability([]byte(s.folder), []byte(osutil.NormalizedFilename(file)))
}
...
func ListFolders(db *leveldb.DB) []string {
	return newLDB(db).ldbListFolders()
}

// DropFolder clears out all information related to the given folder from the
// database.
func DropFolder(db *leveldb.DB, folder string) {
	newLDB(db).ldbDropFolder([]byte(folder))
	bm := &BlockMap{
		db:     db,
		folder: folder,
	}
