...
func NewFileSet(folder string, db *bolt.DB) *FileSet {
	var s = FileSet{
...
func ListFolders(db *bolt.DB) []string {
	bdb := boltDB{db}
	return bdb.listFolders()
}

// DropFolder clears out all information related to the given folder from the
// database.
func DropFolder(db *bolt.DB, folder string) {
	bdb := boltDB{db}
	bdb.dropFolder([]byte(folder))
	bm := &BlockMap{
		db:     db,
		folder: []byte(folder),
	}
