...
type backend interface {
	availability(folder, file []byte) []protocol.DeviceID
	dropFolder(folder []byte)
	get(folder, device, file []byte) (protocol.FileInfo, bool)
	getGlobal(folder, file []byte, truncate bool) (FileIntf, bool)
	listFolders() []string
	replace(folder, device []byte, fs []protocol.FileInfo) int64
	replaceWithDelete(folder, device []byte, fs []protocol.FileInfo, myID uint64) int64
	update(folder, device []byte, fs []protocol.FileInfo) int64
	withAllFolderTruncated(folder []byte, fn func(device []byte, fi FileInfoTruncated) bool)
	withGlobal(folder, prefix []byte, truncate bool, fn Iterator)
	withHave(folder, device []byte, truncate bool, fn Iterator)
	withNeed(folder, device []byte, truncate bool, fn Iterator)
}
...
func NewFileSet(folder string, db backend) *FileSet {
	var s = FileSet{
	...
	if ldb, ok := db.(*ldbBackend); ok {
		ldb.checkGlobals([]byte(folder))
	}
...
func ListFolders(db *leveldb.DB) []string {
	return newLdbBackend(db).listFolders()
}

// DropFolder clears out all information related to the given folder from the
// database.
func DropFolder(db *leveldb.DB, folder string) {
	newLdbBackend(db).dropFolder([]byte(folder))
	bm := &BlockMap{
		db:     db,
		folder: folder,
	}
